...
resource "aws_instance" "aws-vm" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      ...
      "sudo SELF_PUBLIC_IP=$(curl http://169.254.169.254/latest/meta-data/public-ipv4) && sed -i -e \"s/^ip_sans=.*/ip_sans=$SELF_PUBLIC_IP/g\" /etc/consul-template.d/hashi-cert.tpl && sed -i -e \"s/^ip_sans=.*/ip_sans=$SELF_PUBLIC_IP/g\" /etc/consul-template.d/hashi-key.tpl",
